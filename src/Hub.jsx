...
// ════════════════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════════════════
// MODULE 4 — SIMULATEUR RAPIDE DE FICHES CEE
// ════════════════════════════════════════════════════════════════════════════
...
function SimulateurRapide() {
  const [ficheActive, setFicheActive] = useState('BAT-TH-116');
  const [classe,    setClasse]    = useState('A');
  const [secteur,   setSecteur]   = useState('bureaux');
  const [zone,      setZone]      = useState('H2');
  const [bonif,     setBonif]     = useState('none');
  const [surfs,     setSurfs]     = useState({ chauffage:'', refroidissement:'', ecs:'', eclairage:'', auxiliaires:'' });
  const setSurf = (k, v) => setSurfs(s => ({...s, [k]:v}));

  const kwhBase = calculerCumac116Hub({ classe, secteur, zone, surfaces: surfs });
  const kwh     = Math.round(kwhBase * BONIF_COEFF_116[bonif]);

  const INP = {boxSizing:"border-box",background:C.bg,border:`1px solid ${C.border}`,borderRadius:7,padding:"7px 28px 7px 8px",color:C.text,fontSize:12,outline:"none",fontFamily:"inherit",width:"100%"};

  return (
    <div style={{height:"100%",overflowY:"auto",padding:"24px 28px",background:C.bg}}>
      <div style={{maxWidth:760,margin:"0 auto"}}>
        <div style={{marginBottom:20}}>
          <h2 style={{...T.h2,margin:0,marginBottom:4}}>📐 Simulateur de fiches CEE</h2>
          <p style={{...T.sm,margin:0}}>Calculez le volume CEE (kWh cumac) selon la fiche applicable</p>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:24}}>
          {FICHES_SIM.map(f => (
            <button key={f.id} type="button"
              onClick={() => !f.coming && setFicheActive(f.id)}
              disabled={!!f.coming}
              style={{padding:"14px 10px",borderRadius:10,cursor:f.coming?"default":"pointer",fontFamily:"inherit",textAlign:"center",
                position:"relative",opacity:f.coming?0.55:1,
                background:ficheActive===f.id?"#EFF6FF":C.surface,
                border:`2px solid ${ficheActive===f.id?C.accent:C.border}`}}>
              {f.coming && (
              ...
              <div style={{fontSize:11,fontWeight:800,color:ficheActive===f.id?"#2563EB":f.coming?C.textSoft:C.text}}>{f.label}</div>
              <div style={{fontSize:9,color:C.textSoft,marginTop:2,lineHeight:1.3}}>{f.desc}</div>
            </button>
          ))}
        </div>

        {/* Simulateur BAT-TH-116 */}
        {ficheActive === 'BAT-TH-116' && (
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"20px 24px"}}>
            <div style={{fontSize:15,fontWeight:800,color:C.text,marginBottom:16}}>🖥️ BAT-TH-116 — GTB / BMS tertiaire</div>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:16}}>
              {/* Classe */}
              <div>
                <label style={{display:"block",fontSize:11,fontWeight:600,color:C.textMid,marginBottom:6,textTransform:"uppercase",letterSpacing:.4}}>Classe GTB</label>
                <div style={{display:"flex",gap:6}}>
                  {["A","B"].map(c=>(
                    <button key={c} type="button" onClick={()=>setClasse(c)}
                      style={{flex:1,padding:"8px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:700,
                        background:classe===c?"#BFDBFE":C.bg,border:`1px solid ${classe===c?"#2563EB":C.border}`,color:classe===c?"#2563EB":C.textMid}}>
                      Classe {c}
                    </button>
                  ))}
                </div>
                  ...
                  {["H1","H2","H3"].map(z=>(
                    <button key={z} type="button" onClick={()=>setZone(z)}
                      style={{flex:1,padding:"8px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700,
                        background:zone===z?"#BFDBFE":C.bg,border:`1px solid ${zone===z?"#2563EB":C.border}`,color:zone===z?"#2563EB":C.textMid}}>
                      {z}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:5}}>
                  {[{v:"none",l:"Aucune ×1"},{v:"creation",l:"Création ×2"},{v:"amelioration",l:"×1,5"}].map(b=>(
                    <button key={b.v} type="button" onClick={()=>setBonif(b.v)}
                      style={{flex:1,padding:"6px 3px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",fontSize:10,fontWeight:700,
                        background:bonif===b.v?"#BFDBFE":C.bg,border:`1px solid ${bonif===b.v?"#2563EB":C.border}`,color:bonif===b.v?"#2563EB":C.textMid}}>
                      {b.l}
                    </button>
                  ))}
                </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:6}}>
                {SECTEURS_116.map(s=>(
                  <button key={s.id} type="button" onClick={()=>setSecteur(s.id)}
                    style={{padding:"7px 5px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",textAlign:"center",
                      background:secteur===s.id?"#EFF6FF":C.bg,border:`1px solid ${secteur===s.id?"#2563EB":C.border}`}}>
                    <div style={{fontSize:10,fontWeight:700,color:secteur===s.id?"#2563EB":C.text}}>{s.label}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Surfaces par usage */}
            <div style={{marginBottom:16}}>
              <label style={{display:"block",fontSize:11,fontWeight:600,color:C.textMid,marginBottom:8,textTransform:"uppercase",letterSpacing:.4}}>Surfaces gérées par usage (m²)</label>
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
                {USAGES_116.map(u => {
                  const coeff = COEFFICIENTS_116[classe]?.[secteur]?.[u.key];
                  return (
                    <div key={u.key}>
                      <div style={{marginBottom:4}}>
                        <span style={{fontSize:11,color:C.text,fontWeight:600}}>{u.label}</span>
                        {coeff != null && (
                          <span style={{display:"block",fontSize:10,fontWeight:700,color:"#2563EB",background:"#EFF6FF",borderRadius:4,padding:"1px 4px",marginTop:2,width:"fit-content"}}>
                            {coeff} kWh/m²
                          </span>
                        )}
                      </div>
                      <div style={{position:"relative"}}>
                        <input type="number" min="0" value={surfs[u.key]} onChange={e=>setSurf(u.key,e.target.value)} placeholder="0" style={INP}/>
                        <span style={{position:"absolute",right:7,top:"50%",transform:"translateY(-50%)",fontSize:10,color:C.textMid}}>m²</span>
                      </div>
                    </div>
                  );
                })}
            ...
            {kwh > 0 ? (
              <div style={{background:"#F0FDF4",border:"1px solid #86EFAC",borderRadius:10,padding:"16px 20px",textAlign:"center"}}>
                <div style={{fontSize:11,color:"#15803D",marginBottom:4,textTransform:"uppercase",fontWeight:700,letterSpacing:.5}}>Volume CEE estimé</div>
                <div style={{fontSize:28,fontWeight:900,color:"#15803D"}}>{kwh.toLocaleString("fr-FR")} <span style={{fontSize:16,fontWeight:600}}>kWh cumac</span></div>
                {bonif !== 'none' && (
                  <div style={{fontSize:11,color:"#4ADE80",marginTop:4}}>
                    Base : {kwhBase.toLocaleString("fr-FR")} × {BONIF_COEFF_116[bonif]}
                  </div>
                )}
              </div>
            ) : (
              <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"16px",textAlign:"center",color:C.textSoft,fontSize:13}}>
                Saisissez au moins une surface pour calculer le volume CEE
              </div>
            )}
          </div>
        )}
...
const MODULES = [
  ...
  {id:"simulateur",  icon:"📐",titre:"Simulateur de fiches",sousTitre:"Calcul kWh cumac",        desc:"Calculez le volume CEE (kWh cumac) de vos opérations : GTB BAT-TH-116, et bientôt BAT-TH-139, BAT-TH-134, BAT-EN-107.",tags:["Cumac","Simulation","kWh","CEE"],couleur:"#16A34A",actif:true},
];
...
const renderModule = (page, prefill) => {
  ...
  if (page === "simulateur")   return <SimulateurRapide />;
  return null;
};
