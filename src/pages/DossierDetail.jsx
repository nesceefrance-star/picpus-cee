...
export default function DossierDetail() {
  ...
  // Visites techniques liées
  const [visites, setVisites] = useState([])
  const [visiteLightbox, setVisiteLightbox] = useState(null) // { url, label }
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 24px 48px' }}>
        ...
        <div style={{ display: 'flex', gap: 2, marginBottom: 20, borderBottom: `2px solid ${C.border}` }}>
          {[
            ...
            { id: 'visites',   label: '🔧 Visites',    badge: visites.length > 0 ? String(visites.length) : null },
        ...
        {activeTab === 'dossier' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 16, alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <>
                    ...
                    <div style={{ position: 'relative', margin: '10px 0 4px' }}>
                      ...
                      <input
                        ...
                        onBlur={() => setTimeout(() => setAdresseSiteSugg([]), 350)}
                      ...
                      {adresseSiteSugg.length > 0 && (
                        <div
                          ...
                          {adresseSiteSugg.map((f, i) => (
                            <div key={i}
                              onClick={() => selectAdresseSite(f)}
                              onTouchEnd={e => { e.preventDefault(); selectAdresseSite(f) }}
              ...
              <CadastreMap
                dossierId={id}
                adresse={adresseSiteLabel || adresseSite || [dossier.prospects?.adresse, dossier.prospects?.code_postal, dossier.prospects?.ville].filter(Boolean).join(', ')}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {visites.length === 0 ? (
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '40px 22px', textAlign: 'center' }}>
                ...
                <button
                  onClick={() => navigate(`/visites/new?dossier=${id}`)}
                ...
                >+ Créer une visite technique</button>
              </div>
            ) : (
              <>
                ...
                {visites.map(v => {
                  ...
                  const nom = v.donnees?.nom_site || v.donnees?.raison_sociale || dossier.prospects?.raison_sociale || ''
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(90px, 1fr))', gap: 6 }}>
                            {photos.map(photo => (
                              <div key={photo.id} style={{ position: 'relative', borderRadius: 7, overflow: 'hidden', aspectRatio: '1', background: C.bg, cursor: 'pointer' }}
                                onClick={() => setVisiteLightbox({ url: photo.url, label: photo.nom })}>
            ...
            {/* Lightbox photos visite */}
            {visiteLightbox && (
              <div onClick={() => setVisiteLightbox(null)}
                ...
                <img src={visiteLightbox.url} alt={visiteLightbox.label} style={{ maxWidth: '100%', maxHeight: '90vh', borderRadius: 8, objectFit: 'contain' }} />
                <button onClick={() => setVisiteLightbox(null)}
