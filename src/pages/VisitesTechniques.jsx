...
export default function VisitesTechniques() {
  ...
  const handleNew = () => navigate('/visites/new')
  ...
  return (
    <div style={{ padding: isMobile ? '16px 12px' : '28px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: isMobile ? 18 : 22, fontWeight: 800, color: C.text }}>Visites techniques</h1>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: C.textSoft }}>{visites.length} visite{visites.length !== 1 ? 's' : ''} au total</p>
        </div>
        <button
          onClick={handleNew}
          style={{ background: '#16A34A', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}
        >
          + Nouvelle visite
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
        <input
          ...
          style={{ flex: 1, minWidth: 160, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 7, padding: '9px 12px', fontSize: 13, color: C.text, outline: 'none', fontFamily: 'inherit' }}
        />
        <select
          value={filterStatut}
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map(v => {
            ...
            const fiches = v.donnees?.fiches?.length ? v.donnees.fiches : [v.type_fiche].filter(Boolean)
            return (
              <div key={v.id} onClick={() => navigate(`/visites/${v.id}`)}
                style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '14px 16px', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{nom}</div>
                    {v.donnees?.adresse_site && <div style={{ fontSize: 11, color: C.textSoft, marginTop: 2 }}>{v.donnees.adresse_site}</div>}
                    {v.dossiers?.ref && <div style={{ fontSize: 11, color: C.accent, marginTop: 2 }}>📁 {v.dossiers.ref}</div>}
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, color: C.textMid }}>{(v.photos || []).length} 📷 · {updatedAt}</span>
                  {profile?.role === 'admin' && (
                    <div onClick={e => e.stopPropagation()}>
                      <select value={v.assigne_a || ''} onChange={e => handleReassign(e, v.id, e.target.value)}
                        style={{ fontSize: 12, border: `1px solid ${C.border}`, borderRadius: 6, padding: '3px 6px', background: C.surface, color: C.text, fontFamily: 'inherit', cursor: 'pointer' }}>
                  ...
                  <button onClick={e => handleDelete(e, v.id)}
                    style={{ background: 'transparent', border: 'none', color: C.textSoft, cursor: 'pointer', fontSize: 15, padding: 4 }}>🗑</button>
                </div>
              </div>
            )
          })}
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
          ...
          {filtered.map((v, i) => {
            ...
            return (
              <div key={v.id} onClick={() => navigate(`/visites/${v.id}`)}
                style={{ display: 'grid', gridTemplateColumns: cols, gap: 0, padding: '13px 18px', cursor: 'pointer', borderBottom: i < filtered.length - 1 ? `1px solid ${C.bg}` : 'none', alignItems: 'center', transition: 'background .1s' }}
                ...
                <span style={{ fontSize: 12, color: C.textMid }}>{(v.photos || []).length} 📷</span>
