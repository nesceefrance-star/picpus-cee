    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: "system-ui,'Segoe UI',Arial,sans-serif" }}>
      ...
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          {isAdmin && (
            <button onClick={() => navigate('/admin/users')} style={{ background: '#F59E0B22', border: '1px solid #F59E0B44', color: '#F59E0B', borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              ⚙️ Admin
            </button>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => navigate('/hub')} style={{ background: 'transparent', border: `1px solid ${C.border}`, color: C.textMid, borderRadius: 7, padding: '5px 12px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
            🔧 Outils
          </button>
          <span style={{ fontSize: 12, color: C.textMid }}>{profile?.prenom || user?.email}</span>
          {profile?.role && (
            <span style={{ fontSize: 10, fontWeight: 700, color: profile.role === 'admin' ? '#F59E0B' : '#60A5FA', background: profile.role === 'admin' ? '#F59E0B22' : '#60A5FA22', borderRadius: 10, padding: '2px 8px' }}>
              {profile.role.toUpperCase()}
            </span>
          )}
          <button onClick={signOut} style={{ background: 'transparent', border: `1px solid ${C.border}`, color: C.textMid, borderRadius: 7, padding: '5px 12px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
            Déconnexion
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 800, color: C.text, margin: 0, marginBottom: 4 }}>Pipeline CEE</h1>
            <p style={{ fontSize: 13, color: C.textMid, margin: 0 }}>
              {dossiers.length} dossier{dossiers.length > 1 ? 's' : ''} · Prime totale estimée : <strong style={{ color: '#60A5FA' }}>{totalPrimes.toLocaleString('fr')} €</strong>
            </p>
          </div>
          <button onClick={() => setShowWizard(true)} style={{ background: C.accent, color: '#fff', border: 'none', borderRadius: 9, padding: '11px 20px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
            ➕ Nouveau dossier
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 10, marginBottom: 20 }}>
          {STATUTS.map(s => (
            <div key={s.id} onClick={() => setFiltreStatut(filtreStatut === s.id ? 'all' : s.id)}
              style={{ background: filtreStatut === s.id ? s.bg : C.surface, border: `1px solid ${filtreStatut === s.id ? s.color : C.border}`, borderRadius: 10, padding: '12px 14px', cursor: 'pointer', transition: 'all .15s' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ flex: 1, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 9, padding: '10px 16px', color: C.text, fontSize: 14, outline: 'none', fontFamily: 'inherit' }}
          />
          {isAdmin && (
            <select value={filtreCommercial} onChange={e => setFiltreCommercial(e.target.value)}
              style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 9, padding: '10px 14px', color: C.text, fontSize: 13, outline: 'none', fontFamily: 'inherit', minWidth: 180 }}>
        ...
        {loading ? (
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '130px 1fr 140px 140px 130px 100px 80px', gap: 12, padding: '8px 16px', fontSize: 11, fontWeight: 700, color: C.textSoft, textTransform: 'uppercase', letterSpacing: .4 }}>
              <span>Référence</span><span>Prospect</span><span>Fiche CEE</span><span>Commercial</span><span>Prime estimée</span><span>Statut</span><span>Date</span>
            </div>
            {filtered.map(d => {
              const commercial = profiles.find(p => p.id === d.assigne_a)
              return (
                <div key={d.id} onClick={() => openDossier(d)}
                  style={{ display: 'grid', gridTemplateColumns: '130px 1fr 140px 140px 130px 100px 80px', gap: 12, padding: '14px 16px', background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, cursor: 'pointer', marginBottom: 6, transition: 'border-color .15s' }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = '#475569'}
                  onMouseLeave={e => e.currentTarget.style.borderColor = C.border}
                >
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#60A5FA', fontFamily: 'monospace', alignSelf: 'center' }}>{d.ref}</span>
                  <div style={{ alignSelf: 'center' }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{d.prospects?.raison_sociale || '—'}</div>
                    <div style={{ fontSize: 11, color: C.textMid }}>{d.prospects?.ville || d.prospects?.adresse || ''}</div>
                  </div>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 700, color: d.prime_estimee ? '#4ade80' : C.textSoft, alignSelf: 'center' }}>
                    {d.prime_estimee ? `${d.prime_estimee.toLocaleString('fr')} €` : '—'}
                  </span>
                  <span style={{ alignSelf: 'center' }}><StatutBadge statut={d.statut} /></span>
                  <span style={{ fontSize: 11, color: C.textSoft, alignSelf: 'center' }}>{new Date(d.created_at).toLocaleDateString('fr-FR')}</span>
                </div>
              )
            })}
          </div>
